...
uint64_t MAX_INTEGER_LENGTH    = 20;
uint64_t MAX_HEX_INTEGER_LENGTH = 16;  // maximum number of characters in an unsigned integer
...
void init_scanner () {
    ...
    *(SYMBOLS + SYM_RSH)          = (uint64_t) ">>";
    *(SYMBOLS + SYM_LSH)          = (uint64_t) "<<";
...
// | 4 | type    | UINT64_T, UINT64STAR_T, VOID_T, STRUCT
...
uint64_t OP_OP     = 51;  // 0110011, R format (ADD, SUB, MUL, DIVU, REMU, SLTU, AND, OR, SLL, SRL)
...
uint64_t hex_atoi(char* s) {
    ...
    while (c != 0) {
        ...
        if (is_letter(c))
            ...
            c = c - 55; //ignoring lower case, because those should not get here anyway...
...
uint64_t compile_factor() {
    ...
    if (symbol == SYM_IDENTIFIER) {
        ...
        if (symbol == SYM_LPARENTHESIS) {
            ...
            type = load_array(variable_or_procedure_name, 0);
        }
        // integer literal?
    } else if (symbol == SYM_INTEGER) {
    ...
    if(symbol == SYM_LBRACE) {
        while(symbol != SYM_RBRACE) {
            ...
            if(symbol == SYM_UINT64) {
                ...
                data_size = data_size + WORDSIZE;
                create_symbol_table_entry(GLOBAL_TABLE, variable_or_procedure_name, current_line_number, VARIABLE, type, 0, -data_size);
            }
            else if(symbol == SYM_ASTERISK) {
                get_symbol();
...
void compile_statement() {
    ...
    if(array_size == 1) {} // doesnt work otherwise. Could not find a better solution
    ...
    if (symbol == SYM_ASTERISK) {
        ...
        if (symbol == SYM_LPARENTHESIS) {
            ...
            if(is_array == 1) {
                entry = get_variable_or_big_int(variable_or_procedure_name, ARRAY);
            }
...
uint64_t* compile_variable(uint64_t offset) {
    ...
    if (symbol == SYM_IDENTIFIER) {
        ...
        if(symbol == SYM_LBRACKET) {
            ...
            entry = create_symbol_table_entry(LOCAL_TABLE, identifier, line_number, ARRAY, type, 0, offset);
        } else {
        ...
        if (symbol == SYM_VOID) {
            ...
            if (symbol == SYM_IDENTIFIER) {
                ...
                if (symbol == SYM_LPARENTHESIS)
                    ...
                    if(is_array == 1)
                        entry = search_global_symbol_table(variable_or_procedure_name, ARRAY);
                    else
                    ...
                    if (entry == (uint64_t*) 0) {
                        if(is_array == 1) {
                            ...
                            create_symbol_table_entry(GLOBAL_TABLE, variable_or_procedure_name, current_line_number, ARRAY, type, initial_value, -data_size);
                        } else {// allocate memory for global variable in data segment
                            data_size = data_size + WORDSIZE;
...
void selfie_compile() {
    ...
    while (link) {
        if (number_of_remaining_arguments() == 0)
            ...
            printf("%s: %lu calls, %lu assignments, %lu while,  %lu for, %lu if, %lu return\n", selfie_name,
                   number_of_calls,
...
uint64_t get_total_number_of_instructions() {
    return ic_lui + ic_addi + ic_add + ic_xori + ic_and + ic_or + ic_sll + ic_srl + ic_sub + ic_mul + ic_divu + ic_remu + ic_sltu + ic_load + ic_store + ic_beq + ic_jal + ic_jalr + ic_ecall;
}

uint64_t get_total_number_of_nops() {
    return nopc_lui + nopc_addi + nopc_xori + nopc_add + nopc_and + nopc_or + nopc_sll + nopc_srl + nopc_sub + nopc_mul + nopc_divu + nopc_remu + nopc_sltu + nopc_load + nopc_store + nopc_beq + nopc_jal + nopc_jalr;
}
...
uint64_t selfie(uint64_t extras) {
    if (number_of_remaining_arguments() == 0)
        ...
        printf("%s: this is the selfie system from Osman Kocak with\n", selfie_name, SELFIE_URL);
...
int main(int argc, char** argv) {
    ...
    init_system();
    init_target();
    printf("%s: This is Osman Kocak Selfie!\n",selfie_name);
    exit_code = selfie(0);
